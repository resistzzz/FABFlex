...
if accelerator.is_main_process:
    ...
    for data in test_loader:
        ...
        # 后处理小分子
        post_optim_mol(args, pdb_list, accelerator, data,
                       compound_coords_pred, pred_ligand_coord_list, offset_coord_list, init_ligand_coord_list,
                       compound_batch, ligand_rmsd_list, before_rmsd_list, ref_ligand_coord_list, LAS_tmp=LAS_tmp)
    ...
    if args.save_rmsd:
        with open(args.rmsd_file, 'w') as f:
            f.write("pdb\tLigand RMSD\tBefore Ligand RMSD\n")
            for i in range(len(pdb_list)):
                pdb = pdb_list[i]
                rmsd_value = str(ligand_rmsd_list[i])
                before_rmsd_value = str(before_rmsd_list[i])
                f.write(f'{pdb}\t{rmsd_value}\t{before_rmsd_value}\n')
            f.write(f"rmsd: {mean_rmsd}, rmsd < 2A: {rmsd_2A}, rmsd < 5A: {rmsd_5A}, rmsd 25%: {rmsd_25}, rmsd 50%: {rmsd_50}, rmsd 75%: {rmsd_75}")

    args.write_mol_to_file = True
