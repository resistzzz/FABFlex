...
class FABindPlusTwoComplexModel(nn.Module):
    ...
    def forward(self, data, stage=1, train=False, flag=1):
        ...
        gt_keepNode = data['pocket'].keepNode.clone()
        ...
        if self.args.use_iterative:
            ...
            for iter_i in range(total_iter):
                if iter_i < total_iter - 1:
                    with torch.no_grad():
                        # ligand model
                        f_ligand_coords, _, _ = self.complex_model(
                        ...
                        # protein model
                        g_protein_coords, _, _ = self.complex_model_pro(
                            batched_complex_coord.clone(),
                            new_samples,
                            batch_id=complex_batch,
                            segment_id=data['complex'].segment,
                        ...
                        batched_complex_coord[compound_flag] = f_ligand_coords[compound_flag]
                        batched_complex_coord[protein_flag] = g_protein_coords[protein_flag]
                else:
        ...
        if (not self.args.use_iterative) and (not self.args.use_stepbystep):
            # ligand model
            f_ligand_coords, f_ligand_out, f_pair_embed_batched = self.complex_model(
                batched_complex_coord.clone(),
                new_samples,
        ...
        if self.args.use_stepbystep:
            ...
            for iter_i in range(total_iter):
                if iter_i < total_iter - 1:
                    with torch.no_grad():
                        # ligand model
                        aa = batched_complex_coord.clone()
                        f_ligand_coords, _, _ = self.complex_model(
                    ...
                    # protein model
                    g_protein_coords, g_protein_out, g_pair_embed_batched = self.complex_model_pro(
                        batched_complex_coord.clone(),  # 注意这样才能保证f_coords和g_coords不一致
                        new_samples,
                        batch_id=complex_batch,
                        segment_id=data['complex'].segment,
                        lig_mask=data['complex'].lig_mask,
        ...
        return (compound_coords_out, compound_batch, pocket_coords_out, pocket_batch,
                ...
                pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred, pocket_center_bias, gt_pocket_batch, gt_keepNode)

    def inference(self, data, stage=1):
        keepNode_less_5 = 0
        compound_batch = data['compound'].batch
        pocket_batch = data['pocket'].batch
        complex_batch = data['complex'].batch
...
def get_model(args, logger):
    ...
    model = FABindPlusTwoComplexModel(args, args.hidden_size, args.pocket_pred_hidden_size)
    return model
