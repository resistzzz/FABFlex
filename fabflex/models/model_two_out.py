...
class FABindPlusTwoModel(nn.Module):
    def __init__(self, args, embedding_channels=128, pocket_pred_embedding_channels=128):
        ...
        self.complex_model_pro = EfficientMCAttModel(
            args, embedding_channels, embedding_channels, n_channel, n_edge_feats=0, n_layers=args.mean_layers,
            n_iter=args.n_iter,
            inter_cutoff=args.inter_cutoff, intra_cutoff=args.intra_cutoff, normalize_coord=self.normalize_coord,
            unnormalize_coord=self.unnormalize_coord,
    ...
    def iterative_run(self, data, stage=1, train=False, flag=1, is_iter=False):
        keepNode_less_5 = 0
        ...
        if train or stage == 1:
            ...
            for i in range(pred_pocket_center.shape[0]):
                ...
                # keepNode = pred_index_one_hot_true[i][protein_out_mask_whole[i]].round().squeeze(-1)
                # keepNode = keepNode.bool()
        ...
        # ligand model
        f_ligand_coords, f_ligand_out, f_pair_embed_batched = self.complex_model(
            batched_complex_coord.clone(),
            new_samples.clone(),
        ...
        # protein model
        g_protein_coords, g_protein_out, g_pair_embed_batched = self.complex_model_pro(
            batched_complex_coord.clone(),
            new_samples.clone(),
            batch_id=complex_batch,
            segment_id=data['complex'].segment,
        ...
        pocket_out = g_protein_out[protein_flag]
        pocket_coords_out = g_protein_coords[protein_flag].squeeze(-2)
        compound_out = f_ligand_out[compound_flag]
        compound_coords_out = f_ligand_coords[compound_flag].squeeze(-2)
        ...
        f_z = f_pair_embed_batched[:, 1:, 1:, ...]
        g_z = g_pair_embed_batched[:, 1:, 1:, ...]
        z_mask = torch.einsum("bi,bj->bij", pocket_out_mask, compound_out_mask)

        f_b = self.distmap_mlp(f_z).squeeze(-1)
        g_b = self.distmap_mlp(g_z).squeeze(-1)
        f_y_pred = f_b[z_mask]
        g_y_pred = g_b[z_mask]
        f_y_pred = f_y_pred.sigmoid() * self.args.dis_map_thres  # normalize to 0 to 10.
        g_y_pred = g_y_pred.sigmoid() * self.args.dis_map_thres
        ...
        if is_iter:
            return compound_coords_out, pocket_coords_out
        else:
            return (compound_coords_out, compound_batch, pocket_coords_out, pocket_batch,
                    f_y_pred, g_y_pred, y_pred_by_coords, pocket_cls_pred, pocket_cls, protein_out_mask_whole,
                    protein_coords_batched_whole,
                    pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred)

    def forward(self, data, stage=1, train=False, flag=1):
        if self.args.use_iterative:
            if self.args.random_total_iter and self.training:
                total_iter = random.randint(1, self.args.total_iterative)
            else:
                total_iter = self.args.total_iterative
            for iter_i in range(total_iter):
                if iter_i < total_iter - 1:
                    with torch.no_grad():
                        compound_coords_out, pocket_coords_out = self.iterative_run(data, stage, train, flag, is_iter=True)

                        # iterative
                        # data.input_pocket_node_xyz = pocket_coords_out
                        data['compound'].node_coords = compound_coords_out
                        # data.input_protein_node_xyz[data['pocket'].keepNode] = pocket_coords_out

                        compound_flag = torch.logical_and(data["complex"].segment == 0, ~data["complex"].is_global)
                        data['complex'].node_coords[compound_flag] = compound_coords_out
                        pocket_flag = torch.logical_and(data["complex"].segment == 1, ~data["complex"].is_global)
                        data['complex'].node_coords[pocket_flag] = pocket_coords_out

                        compound_flag = torch.logical_and(data["complex_whole_protein"].segment == 0,
                                                          ~data["complex_whole_protein"].is_global)
                        data['complex_whole_protein'].node_coords[compound_flag] = compound_coords_out
                        protein_flag = torch.logical_and(data["complex_whole_protein"].segment == 1,
                                                         ~data["complex_whole_protein"].is_global)
                        indices_of_true = torch.where(protein_flag)[0]
                        selected_indices = indices_of_true[data["pocket"].keepNode.bool()]
                        data['complex_whole_protein'].node_coords[selected_indices] = pocket_coords_out
                else:
                    (compound_coords_out, compound_batch, pocket_coords_out, pocket_batch,
                     f_y_pred, g_y_pred, y_pred_by_coords, pocket_cls_pred, pocket_cls, protein_out_mask_whole,
                     protein_coords_batched_whole,
                     pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred) = self.iterative_run(data, stage, train, flag, is_iter=False)
        else:
            (compound_coords_out, compound_batch, pocket_coords_out, pocket_batch,
             f_y_pred, g_y_pred, y_pred_by_coords, pocket_cls_pred, pocket_cls, protein_out_mask_whole,
             protein_coords_batched_whole,
        ...
        return (compound_coords_out, compound_batch, pocket_coords_out, pocket_batch,
                f_y_pred, g_y_pred, y_pred_by_coords, pocket_cls_pred, pocket_cls,
                protein_out_mask_whole, protein_coords_batched_whole,
                pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred)
...
def get_model(args, logger):
    ...
    model = FABindPlusTwoModel(args, args.hidden_size, args.pocket_pred_hidden_size)
    return model
