...
from models.model_utils import Attention, permute_final_dims, MLPwithLastAct, InteractionModule


class CrossAttentionModule(nn.Module):
    def __init__(self, args, node_hidden_dim, pair_hidden_dim,
                 rm_layernorm=False, keep_trig_attn=False, dist_hidden_dim=32, normalize_coord=None):
        super().__init__()
        self.pair_hidden_dim = pair_hidden_dim
        self.keep_trig_attn = keep_trig_attn
        self.p_attention_block = RowAttentionBlock(args, node_hidden_dim, pair_hidden_dim,
                                                   no_heads=args.mha_heads, rm_layernorm=rm_layernorm, mha_permu=True)
        self.c_attention_block = RowAttentionBlock(args, node_hidden_dim, pair_hidden_dim,
                                                   no_heads=args.mha_heads, rm_layernorm=rm_layernorm, mha_permu=False)
        self.p_transition = MLPwithLastAct(args, embedding_channels=node_hidden_dim,
                                           n=args.mlp_hidden_scale, out_channels=node_hidden_dim)
        self.c_transition = MLPwithLastAct(args, embedding_channels=node_hidden_dim,
                                           n=args.mlp_hidden_scale, out_channels=node_hidden_dim)
        self.pair_transition = MLPwithLastAct(args, embedding_channels=pair_hidden_dim,
                                              n=args.mlp_hidden_scale, out_channels=pair_hidden_dim)
        self.inter_layer = InteractionModule(node_hidden_dim, pair_hidden_dim, 32,
                                             opm=False, rm_layernorm=rm_layernorm)

    def forward(self, p_embed_batched, p_mask, c_embed_batched, c_mask, pair_embed, pair_mask, distance=None):
...
class RowAttentionBlock(nn.Module):
    def __init__(self, args, node_hidden_dim, pair_hidden_dim, attention_hidden_dim=32,
                 no_heads=4, dropout=0.1, rm_layernorm=False, mha_permu=False):
        super().__init__()
