...
def train_one_epoch(epoch, accelerator, args, logger, train_loader, model, optimizer, scheduler,
                    ...
                    contact_criterion, com_coord_criterion, pro_coord_criterion, device, stage=1, flag=1, prefix="protein-train"):
    batch_count = 0
    batch_loss = 0.0
    ...
    for batch_id, data in enumerate(data_iter, start=1):
        ...
        (compound_coords_pred, compound_batch, pocket_coords_pred, pocket_batch,
         ...
         pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred, _, _, _) = model(data, stage=stage, train=True, flag=flag)

        if pocket_cls_pred.isnan().any() or pred_pocket_center.isnan().any():
            print(f"nan occurs in model of epoch {epoch}- batch {batch_id}")
            continue
        if f_y_pred.isnan().any() or g_y_pred.isnan().any() or y_pred_by_coords.isnan().any() or compound_coords_pred.isnan().any() or pocket_coords_pred.isnan().any():
            print(f"nan occurs in model of epoch {epoch}- batch {batch_id}")
            continue
        ...
        af2_pocket_sd = ((data.af2_protein_node_xyz[data['pocket'].keepNode].detach() - gt_pocket_coord) ** 2).sum(dim=-1)
