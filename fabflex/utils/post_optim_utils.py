...
def post_optimize_compound_coords(reference_compound_coords, predict_compound_coords, gt_compound_coords,
    ...
    loss_list = []
    rmsd_list = []
    before_rmsd_list = compute_RMSD(gt_compound_coords, predict_compound_coords)
    for epoch in range(total_epoch):
        ...
        rmsd = compute_RMSD(gt_compound_coords, x.detach())
        rmsd_list.append(rmsd.item())
        # break
    # return x.detach().cpu().numpy(), loss_list[-1], rmsd_list[-1]
    return x.detach(), loss_list[-1], rmsd_list[-1], before_rmsd_list.item()
